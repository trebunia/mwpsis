...
s.t. c6: sum{raid in RAID, disk in DISK} X[raid, disk] * S[disk] * G[raid] >= S_klienta * sum{raid in RAID, disk in DISK} X[raid, disk];
s.t. c7: sum{raid in RAID, disk in DISK} 8 * X[raid, disk] * V[disk] * R[raid] <= D_max;
#s.t. c8: X[1, 4] <= 0;
...
param S_klienta := 631;
param D_min := 50000000;
param D_max := 50000100;
