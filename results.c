// to compile g++ results.c -o results -lkcgihtml -lkcgi -L/usr/local/lib -lClp -L/usr/local/lib -lcoinglpk -ldl -lm -L/usr/local/lib -lCoinUtils -lm -L/usr/local/lib -lcoinglpk -ldl -lm -I/usr/include/cppconn -L/usr/lib -lmysqlcppconn
...
int print_res() {
sql::ResultSet *res;
res = run_query("select unique_id from optymalizacje order by start_time;");
...
while (res->next()) {
	std::cout << "<p><a href=\"/cgi-bin/opt?RES=" << res->getString("unique_id") << "\"> Execution ID: " << res->getString("unique_id") << "</a></p>";
